...
export async function Parse(host: string) {
    ...
    let functions: ESTree.FunctionDeclaration[] = body.filter(
...
export function ParseSchoolRa(node: ESTree.BlockStatement) {
    ...
    if (
        !((callee.object as Identifier).name == '$') &&
        (callee.property as Identifier).name == 'ajax'
    )
        throw new Error('Parse Failed');
    let argument = ajaxCall.arguments.find(
        (x) => x.type == 'ObjectExpression'
    ) as ObjectExpression;
    let findUrlKey = function (property: SpreadElement | Property) {
        if (property.type != 'Property') return false;
        if (property.key.type != 'Identifier') return false;
        if (property.key.name != 'url') return false;
    };
    ...
    if (left.type != 'Literal') throw new Error('Parse Failed');
    if (!left.raw) throw new Error('Parse Failed');
